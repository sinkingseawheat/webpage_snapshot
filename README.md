...
すみませんがGithubでのプロジェクト管理をやったことがないので、対応はかなり遅くなるか、ほったらかしになるかと思います。
もちろん、勝手にForkして運用するのも問題ないので、本格的に活用をしたい方はそちらの検討をお願いします。
...
Node.jsのインストールは[公式サイト](https://nodejs.org/en)や[Volta](https://volta.sh/)などのバージョン管理ツールなどを利用してインストールしてください。  
git clone、あるいはダウンロードしたファイル一式のルートディレクトリでプロンプトを開いてください  
（windowsを利用している方はエクスプローラでディレクトリを開き、アドレスバーに`cmd`や`powershell`と打ち込んでEnterを押せばOKです。）  

### 1.ファイルのビルドとヘッドレスブラウザのインストール（※ これは最初だけです。）
まずはコンソールで以下のコマンドでnpm_modulesのダウンロードとビルドおよび、ヘッドレスブラウザのダウンロードをしてください。
...
「入力欄」の「対象のURL（必須）」に対象のURL（=targetURL）を改行区切りで、「幅（必須）」にブラウザの幅を、「高さ（必須」にブラウザの高さをそれぞれ入力してください。  
送信ボタンが押せるようになるはずなので「送信」ボタンを押してください。ジョブの処理が開始されます。  
