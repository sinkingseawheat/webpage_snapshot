...
const DescriptionOfOtherFile:React.FC<{
  ...
  return (<div className={style.table}>
    ...
    requestURL, type, tagName, relURL, href, responseURL, status, contentType, contentLength, shaHash, source, linkSourceIndex, archiveIndex, errorMessage
        ...
        return (<tr key={requestURL}>
          <td className={style["table__data--url"]}>{archiveIndex !== null && archiveIndex !== undefined ? <Link target="_blank" className={style.textLink} href={getPath(`archive/${archiveIndex}${query === '' ? '' : `?${query}`}`)}>{requestURL}</Link> : requestURL}</td>
